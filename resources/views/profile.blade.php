...
<body class="min-h-screen px-4 py-5">
    <div class="w-full max-w-lg mx-auto pb-28">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold text-[#111827]">Profile</h1>
                <p class="text-gray-500 text-sm">Kelola informasi dan kode absensi Anda</p>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <button id="disableBiometricBtn" class="hidden px-4 py-2 bg-[#FFF7D6] hover:bg-amber-100 text-amber-700 text-sm rounded-2xl border border-amber-200 transition-all">
                    Nonaktifkan Login Sidik Jari
                </button>
            </div>
        </div>

        <div class="soft-card rounded-[28px] p-6 sm:p-7 space-y-6">
            <div class="flex items-center gap-4">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-300 to-amber-500 shadow-[0_10px_25px_rgba(251,191,36,0.25)]">
                ...
                <div>
                    ...
                    <p id="displayName" class="text-[#111827] font-semibold text-lg">-</p>
                </div>
            </div>

            <div class="grid grid-cols-1 gap-3 text-sm text-gray-600">
                <div class="rounded-2xl bg-[#FFFDF5] p-4 border border-amber-100">
                    <p class="text-gray-500 text-xs uppercase tracking-[0.2em] mb-1">NIK</p>
                    <p id="displayNik" class="text-[#111827] font-medium">-</p>
                </div>
                <div class="rounded-2xl bg-[#FFFDF5] p-4 border border-amber-100">
                    <p class="text-gray-500 text-xs uppercase tracking-[0.2em] mb-1">PIN</p>
                    <p id="displayPin" class="text-[#111827] font-medium">-</p>
                </div>
                <div class="rounded-2xl bg-[#FFFDF5] p-4 border border-amber-100">
                    <p class="text-gray-500 text-xs uppercase tracking-[0.2em] mb-1">Status wajah</p>
                    <p id="displayFaceStatus" class="text-amber-700 font-semibold">Memuat...</p>
                </div>
                <div class="rounded-2xl bg-[#FFFDF5] p-4 border border-amber-100">
                    <p class="text-gray-500 text-xs uppercase tracking-[0.2em] mb-1">Login terakhir</p>
                    <p id="displayLastLogin" class="text-[#111827] font-medium">-</p>
                </div>
                <div class="rounded-2xl bg-[#FFFDF5] p-4 border border-amber-100">
                    <p class="text-gray-500 text-xs uppercase tracking-[0.2em] mb-1">Device ID</p>
                    <p id="displayDeviceId" class="text-[#111827] font-medium break-all">-</p>
                </div>
            </div>

            <div class="rounded-[24px] bg-[#FFFDF5] p-5 border border-amber-100">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <p class="text-[#111827] font-semibold">Ubah Kode Absensi</p>
                        <p class="text-gray-500 text-sm">Gunakan kode 6 digit baru yang mudah diingat.</p>
                    </div>
                </div>
                ...
                <form id="changePinForm" class="space-y-4">
                    <div>
                        <label for="current_pin_absensi" class="block text-sm font-medium text-gray-700 mb-1">Kode Absensi Lama</label>
                        <input type="password" id="current_pin_absensi" name="current_pin_absensi" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="******" required autocomplete="off" class="w-full h-[54px] px-4 bg-white border border-gray-200 rounded-2xl text-[#111827] placeholder-gray-400 text-base tracking-widest focus:outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100 transition-all">
                    </div>
                    <div>
                        <label for="new_pin_absensi" class="block text-sm font-medium text-gray-700 mb-1">Kode Absensi Baru</label>
                        <input type="password" id="new_pin_absensi" name="new_pin_absensi" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="******" required autocomplete="off" class="w-full h-[54px] px-4 bg-white border border-gray-200 rounded-2xl text-[#111827] placeholder-gray-400 text-base tracking-widest focus:outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100 transition-all">
                    </div>
                    <div>
                        <label for="new_pin_absensi_confirmation" class="block text-sm font-medium text-gray-700 mb-1">Konfirmasi Kode Absensi Baru</label>
                        <input type="password" id="new_pin_absensi_confirmation" name="new_pin_absensi_confirmation" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="******" required autocomplete="off" class="w-full h-[54px] px-4 bg-white border border-gray-200 rounded-2xl text-[#111827] placeholder-gray-400 text-base tracking-widest focus:outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100 transition-all">
                    </div>

                    <button type="submit" id="submitBtn" class="w-full h-[54px] bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-[#111827] font-bold rounded-2xl shadow-[0_10px_25px_rgba(251,191,36,0.35)] hover:shadow-[0_14px_30px_rgba(251,191,36,0.45)] active:scale-[0.98] transition-all">
                        Simpan Perubahan
                    </button>
                </form>
            </div>
    ...
    <script>
        ...
        async function updateBiometricButton() {
            const biometricEnrolled = localStorage.getItem('biometric_enrolled') === 'true';
            if (biometricEnrolled && window?.Capacitor?.Plugins?.NativeBiometric) {
                disableBiometricBtn.classList.remove('hidden');
            } else {
                disableBiometricBtn.classList.add('hidden');
            }
        }

        disableBiometricBtn.addEventListener('click', async function () {
            try {
                const nativeBiometric = window?.Capacitor?.Plugins?.NativeBiometric;
                if (nativeBiometric) {
                    await nativeBiometric.deleteCredentials({ server: 'waj-attendance' });
                }
            } catch (err) {
                console.warn('Failed to delete biometric credentials:', err);
            }

            localStorage.removeItem('biometric_enrolled');
            showMessage('success', 'Login sidik jari berhasil dinonaktifkan.');
